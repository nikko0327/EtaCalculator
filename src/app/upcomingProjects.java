...
public class upcomingProjects extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try {
            ...
            if (searchUpcomingProjects()) {
                ...
                System.out.println("JSON: " + searchResult);
            } else {
            ...
            System.out.println("Servlet error in upcoming projects: " + e.getMessage());
    ...
    public boolean searchUpcomingProjects() {
        ...
        try {
            ...
            long counter = db_credentials.DB.getApplianceCount(connect);
            System.out.println("Appliance Count: " + counter); //Test run

            // ./Query to count appliances

            // Counting appliances in use
            long counterUsed = db_credentials.DB.getAppliancesInUseCount(connect);
    ...
    public long daysBetween(Date d1, Date d2) {
        ...
        return (int) ((d2.getTime() - d1.getTime()) / dayInMS);
    }
    ...
    public Map<String, String> getSearchDriveJSONResults(ResultSet rs) {
        ...
        try {
            ...
            map.put("started", "" + rs.getBoolean("start"));
            ...
            String expected_end_month = rs.getDate("expected_end_month") == null ? "" : rs.getDate("expected_end_month").toString();
...
//            cal2.setTime(date);
//            long numOfDays = daysBetween(cal1.getTime(), cal2.getTime()); // This is the one we need to calculate the appliances needed

                Date start = stringToDate(expected_start_month);
                Date end = stringToDate(expected_end_month);
                long numOfDays = daysBetween(start, end);
...
//            final long gigsPerDay = 150; //Number of gigabytes Proofpoint process
//            System.out.println("Number of Gigs Per Day: " + gigsPerDay);
//            long dateNoPadding = numOfDays - 14; // Taking away 2 weeks of padding
//            System.out.println("Date No Padding: " + dateNoPadding);
//            long dateNoMapping = dateNoPadding - 30; // Taking away 30 mapping days
...
//            long totalNumberOfGigsNeeded = (int) Math.ceil(numberOfGigsPerDay);
...
//            map.put("apps_needed", finalOutput);
                ...
                long estimatedSize = rs.getInt("estimated_size");
                long numberOfAppliances = calculateAppliancesNeeded(start, end, estimatedSize);
                ...
                map.put("apps_needed", "Both dates are not set");
            }
        } catch (Exception e) {
            System.out.println("------------------Error in getting drive search results " + e.getMessage());
        }
        return map;
    }

    public long calculateAppliancesNeeded(Date start, Date end, long estimatedSize) {
        long daysBetween = daysBetween(start, end);
        if (daysBetween <= 0) {
            return 1;
        }
        final long gigsPerDay = 150; // Amount of data Proofpoint appliances can process per day.
        long noPadding = daysBetween - 14; // Take away 2 weeks of padding
        if (noPadding < 1) {
            return 1;
        }
        long noMapping = noPadding - 30; // Take away 30 days of mapping
        if (noMapping < 1) {
            return 1;
        }
        noMapping /= 2;
        ...
        long result = (int) Math.ceil(preliminary);
